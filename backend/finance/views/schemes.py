...
class SchemesView(APIView):
    ...
    def get(self, request):
        ...
        # Get user's fields for automatic matching (recent activity)
        ...
        for scheme in SAMPLE_SCHEMES:
            ...
            # --- DYNAMIC "RECENT ACTIVITY" VARIANCES FOR LIVE REFRESH FEEL ---

            # Dynamically recalculate deadlines (shifts dynamically on refresh)
            deadline_days = random.randint(10, 60)
            scheme_copy['application_deadline'] = (timezone.now() + timedelta(days=deadline_days)).date().isoformat()
            
            # Dynamically vary subsidy amounts slightly for live fluctuation effect
            if scheme_copy.get('max_subsidy_amount'):
                variance = random.uniform(0.95, 1.05)
                scheme_copy['max_subsidy_amount'] = int(scheme_copy['max_subsidy_amount'] * variance)

            # Calculate match score based on user's profile and recent activity
            match_score = self._calculate_match_score(scheme_copy, user_crops, state)
            
            # Add small random fluctuation to match score to simulate live recalculation
            match_score += random.randint(-4, 4)
            match_score = max(10, min(100, match_score)) # clamp 10-100
    ...
    def _calculate_match_score(self, scheme, user_crops, state):
        ...
        # Bonus for matching crops (Recent Activity)
        if scheme['eligible_crops']:
            matching_crops = set(user_crops) & set(scheme['eligible_crops'])
            if matching_crops:
                score += 30  # High bonus for matching recent user activity/crops
        else:
        ...
        # Bonus for having active deadline
