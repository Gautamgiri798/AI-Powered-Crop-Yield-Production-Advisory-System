...
def detect_awd_from_ndwi(
    ...
    min_dry_days_for_awd: int = 2,  # Reduced from 3 to be more lenient
    min_cycles: int = 1,
    min_dry_fraction: float = 0.03 # Reduced from 0.05
) -> Dict[str, Any]:
    """
    Detect AWD from an NDWI time series using dry-day counts and wet-dry cycles.
    Includes adaptive thresholding for cases where absolute NDWI is high.
    """
    ...
    # Adaptive Thresholding: If absolute thresholds fail but there are relative drops
    peak = np.max(ndwi)
    bottom = np.min(ndwi)
    
    # If field is very wet (like in screenshot 0.8), use relative drops
    if peak > 0.5 and bottom > dry_threshold:
        # If NDWI drops by more than 20% from peak, consider it a drying attempt
        adaptive_dry = peak * 0.8
        logger.info(f"Using adaptive AWD dry threshold: {adaptive_dry:.2f} (Peak: {peak:.2f})")
    else:
    ...
    state_arr[ndwi <= adaptive_dry] = "dry"
    ...
    return {
        ...
        "notes": final_notes,
        "adaptive_threshold_used": peak > 0.5 and bottom > dry_threshold
    }
...
def calculate_carbon_metrics(
    ...
    if awd_result["awd_detected"]:
        if awd_result["cycles_count"] >= 2 and awd_result["dry_fraction"] >= 0.2:
            ...
            eff = 0.6  # Increased from 0.5
    elif awd_result["total_dry_days_count"] > 0:
        # Partial efficiency even if full AWD not detected
        eff = min(0.3, (awd_result["dry_fraction"] * 0.5))
    ...
    # Potential calculation (if they fully implement AWD)
    potential_eff = 1.0
    pot_methane_reduction_kg = methane_baseline_total_kg * (potential_eff * awd_reduction_factor)
    pot_co2e_reduction_ton = (pot_methane_reduction_kg * ch4_to_co2e) / 1000.0
    pot_value_inr = pot_co2e_reduction_ton * credit_price_inr
    pot_water_saved = baseline_water_mm * water_saving_fraction * area_hectare * 10.0
    ...
    return {
        ...
        "potential": {
            "carbon_credits": round(pot_co2e_reduction_ton, 3),
            "estimated_value_inr": round(pot_value_inr, 2),
            "methane_reduction_kg": round(pot_methane_reduction_kg, 2),
            "water_saved_cubic_m": round(pot_water_saved, 2)
        }
    }