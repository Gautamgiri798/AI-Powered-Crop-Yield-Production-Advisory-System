...
# Cache Configuration - probe Redis availability at startup
def _get_cache_config():
    redis_url = os.environ.get('REDIS_URL', 'redis://127.0.0.1:6379/0')
    try:
        import django_redis
        # Quick connectivity check before committing to Redis cache
        import redis as _redis_lib
        from urllib.parse import urlparse
        parsed = urlparse(redis_url)
        host = parsed.hostname or '127.0.0.1'
        port = parsed.port or 6379
        r = _redis_lib.Redis(host=host, port=port, socket_connect_timeout=2, socket_timeout=2)
        r.ping()
        return {
            'default': {
        ...
        _log.getLogger(__name__).warning(
            ...
            "Throttling will use in-process memory (fine for local dev)."
        )
        return {
            'default': {
                'BACKEND': 'django.core.cache.backends.locmem.LocMemCache',
                'LOCATION': 'krishisaarthi-locmem',
            }
        }
