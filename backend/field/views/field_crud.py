...
class SavePolygon(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            irrigation_type = request.data.get("irrigationType", "")
            area = request.data.get("area", 0)
            ...
            if field_id:
                ...
                field_data.soilType = soil_type
                field_data.irrigationType = irrigation_type
                field_data.area = area
            ...
            return Response(
                {
                    ...
                    "irrigationType": field_data.irrigationType,
...
class getCoord(APIView):
    ...
    def get(self, request):
        ...
        if isinstance(polygon, dict):
            ...
            if raw_coords and isinstance(raw_coords[0], list) and len(raw_coords[0]) > 0:
                point = raw_coords[0][0]
                if isinstance(point, (list, tuple)) and len(point) >= 2:
                    lon, lat = point[0], point[1]
        elif isinstance(polygon, list) and len(polygon) > 0:
            if isinstance(polygon[0], dict):
                p = polygon[0]
                lon, lat = p.get('lng', p.get('lon')), p.get('lat')
